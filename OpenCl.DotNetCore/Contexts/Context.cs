...
namespace OpenCl.DotNetCore.Contexts
{
    ...
    public class Context : HandleBase
    {
        ...
        public MemoryBuffer CreateBuffer(OpenCl.DotNetCore.Memory.MemoryFlag memoryFlags, int size)
        {
        ...
        /// <exception cref="OpenClException">If the memory buffer could not be created, then an <see cref="OpenClException"/> is thrown.</exception>
        /// <returns>Returns the created memory buffer.</returns>
        public MemoryBuffer CreateBuffer<T>(OpenCl.DotNetCore.Memory.MemoryFlag memoryFlags, int size) where T : struct => this.CreateBuffer(memoryFlags, Marshal.SizeOf<T>() * size);
